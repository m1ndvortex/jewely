...
def fixed_asset_list(request):
    ...
    try:
        ...
        total_acquisition_cost = assets.aggregate(total=Sum("acquisition_cost"))[
            "total"
        ] or Decimal("0.00")

        total_accumulated_depreciation = assets.aggregate(total=Sum("accumulated_depreciation"))[
            "total"
        ] or Decimal("0.00")
...
def fixed_asset_detail(request, asset_id):
    ...
    """
    try:
        asset = get_object_or_404(FixedAsset, id=asset_id, tenant=request.user.tenant)

        # Get depreciation schedule
        depreciation_schedules = DepreciationSchedule.objects.filter(fixed_asset=asset).order_by(
            "period_end_date"
        )
...
def fixed_asset_create(request):
    ...
    try:
        ...
        return render(
            request,
            "accounting/fixed_assets/form.html",
            {
                "category_choices": FixedAsset.CATEGORY_CHOICES,
                "depreciation_method_choices": FixedAsset.DEPRECIATION_METHOD_CHOICES,
            },
...
def depreciation_schedule(request):
    ...
    try:
        ...
        schedules = (
            DepreciationSchedule.objects.filter(fixed_asset__tenant=request.user.tenant)
            .select_related("fixed_asset")
            .order_by("period_end_date", "fixed_asset__asset_name")
        )

        # Calculate totals
        total_depreciation = schedules.aggregate(total=Sum("depreciation_amount"))[
            "total"
        ] or Decimal("0.00")

        total_accumulated = schedules.aggregate(total=Sum("accumulated_depreciation"))[
            "total"
        ] or Decimal("0.00")
