    ...
    def decorator(view_func: Callable) -> Callable:
        ...
        def wrapper(request: HttpRequest, *args, **kwargs):
            ...
            if not tenant:
                ...
                if api_response:
                    return JsonResponse(
                        {"error": "No tenant context", "code": "no_tenant"}, status=400
                    )
    ...
    def decorator(view_func: Callable) -> Callable:
        ...
        def wrapper(request: HttpRequest, *args, **kwargs):
            ...
            if not service.check_feature(feature_name):
                ...
                logger.info(f"Feature access denied: tenant={tenant.slug}, feature={feature_name}")
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.subscription_limit_type:
            ...
            result = service.check_limit(
                self.subscription_limit_type, self.subscription_limit_increment
            )
...
class ActiveSubscriptionRequiredMixin:
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not tenant:
            if self.subscription_api_response:
                return JsonResponse({"error": "No tenant context", "code": "no_tenant"}, status=400)
            return HttpResponseForbidden("Tenant context required")
