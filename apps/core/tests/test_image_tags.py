...
class TestImageTags(TestCase):
    ...
    def test_lazy_img_basic(self):
        ...
        template = Template(
            "{% load image_tags %}"
            "{% lazy_img 'test.jpg' alt='Test Image' %}"
        )
        context = Context({})
    ...
    def test_lazy_img_with_css_class(self):
        ...
        template = Template(
            "{% load image_tags %}"
            "{% lazy_img 'test.jpg' alt='Test' css_class='w-full h-auto' %}"
        )
        context = Context({})
        output = template.render(context)
    ...
    def test_lazy_img_with_dimensions(self):
        ...
        template = Template(
            "{% load image_tags %}"
            "{% lazy_img 'test.jpg' alt='Test' width='800' height='600' %}"
        )
        context = Context({})
        output = template.render(context)
    ...
    def test_add_lazy_loading_preserves_existing(self):
        ...
        context = Context({
            'html_content': '<img src="test.jpg" alt="Test" loading="lazy">'
        })
        output = template.render(context)
