...
from apps.core.mixins import TenantRequiredMixin, TenantOwnerRequiredMixin
...
class SubscriptionDashboardView(LoginRequiredMixin, TenantRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if current_subscription:
            ...
            if current_subscription.current_period_end:
                days_remaining = (
                    current_subscription.current_period_end - timezone.now()
                ).days
        ...
        if current_subscription and current_subscription.plan:
            upgrade_plans = SubscriptionPlan.objects.filter(
                ...
                price__gt=current_subscription.plan.price
            ).order_by("price")[:3]
        
        # Get recent purchase history
        recent_purchases = SubscriptionPurchase.objects.filter(
            tenant=tenant
    ...
    def _get_usage_stats(self, tenant, subscription):
        ...
        product_count = InventoryItem.objects.filter(tenant=tenant).count() if hasattr(InventoryItem, 'tenant') else 0
        products_limit = plan.products_limit if plan.products_limit and plan.products_limit > 0 else None
        stats.append({
            "name": _("Products"),
            "current": product_count,
            "limit": products_limit or float("inf"),
            "percentage": min(100, (product_count / products_limit * 100)) if products_limit else 0,
        })
        
        # Customers
        customer_count = Customer.objects.filter(tenant=tenant).count() if hasattr(Customer, 'tenant') else 0
        contacts_limit = plan.contacts_limit if plan.contacts_limit and plan.contacts_limit > 0 else None
        stats.append({
            "name": _("Customers"),
            "current": customer_count,
            "limit": contacts_limit or float("inf"),
            "percentage": min(100, (customer_count / contacts_limit * 100)) if contacts_limit else 0,
        })
...
class PurchaseSubscriptionView(LoginRequiredMixin, TenantOwnerRequiredMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if step == "plan":
            context["form"] = SubscriptionPlanSelectionForm(
            ...
            if not all([context.get("selected_plan"), billing_period, 
    ...
    def post(self, request, *args, **kwargs):
        ...
        if step == "plan":
            ...
            form = SubscriptionPurchaseConfirmForm(
                request.POST, 
                tenant=request.user.tenant
            )
            if form.is_valid():
                with transaction.atomic():
                    ...
                    if purchase:
                        ...
                        if purchase.payment_method == "placeholder":
                            return redirect(
                                "payments:process_placeholder_payment",
            ...
            if plan_id:
                try:
                    ...
                    if billing_period:
                        ...
                        context["pricing"] = self._calculate_pricing(selected_plan, billing_period_int)
...
class ProcessPlaceholderPaymentView(LoginRequiredMixin, TenantOwnerRequiredMixin, View):
    ...
    def get(self, request, purchase_id):
        ...
        messages.success(
            request, 
            _("Payment successful! Your subscription has been activated.")
        )
        return redirect("payments:subscription_dashboard")
...
class PurchaseHistoryView(LoginRequiredMixin, TenantRequiredMixin, ListView):
    ...
    def get_queryset(self):
        tenant = self.request.user.tenant
        return SubscriptionPurchase.objects.filter(
            tenant=tenant
...
class SubscriptionCancellationView(LoginRequiredMixin, TenantOwnerRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        if current_subscription:
            ...
            logger.info(
                f"Subscription cancelled for tenant {tenant.id}: "
                f"reason={reason}"
            )
            
            messages.success(
                self.request, 
                _("Your subscription has been cancelled. You will have access until the end of your billing period.")
            )
        else:
