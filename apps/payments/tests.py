...
def create_test_user(username='testuser', email='test@example.com', password='testpass123', role='PLATFORM_ADMIN'):
...
class SubscriptionDiscountModelTests(TestCase):
    ...
    def test_ordered_discounts(self):
        ...
        discounts = SubscriptionDiscount.objects.filter(is_active=True).order_by('billing_period_months')
...
class SubscriptionFormsTests(TestCase):
    ...
    def test_plan_selection_form_valid(self):
        ...
        form = SubscriptionPlanSelectionForm(data={
            'plan': self.plan.id,
        })
    ...
    def test_subscription_cancellation_form_valid(self):
        ...
        form = SubscriptionCancellationForm(data={
