...
class TestNginxLogging:
    ...
    def test_access_log_format_includes_response_times(self):
        ...
        assert "uct=" in content or "upstream_connect_time" in content, \
            "Upstream connect time not in log format"
        assert "uht=" in content or "upstream_header_time" in content, \
            "Upstream header time not in log format"
        assert "urt=" in content or "upstream_response_time" in content, \
    ...
    def test_error_log_levels_configured(self):
        ...
        assert re.search(r'error_log\s+/var/log/nginx/error\.log\s+warn', content), \
            "Standard error log not configured at warn level"
        
        # Check for critical error log
        assert re.search(r'error_log\s+/var/log/nginx/error-critical\.log\s+crit', content), \
    ...
    def test_logging_snippet_included_in_nginx_conf(self):
        ...
        assert "include /etc/nginx/snippets/logging.conf" in content, \
...
class TestNginxMetrics:
    ...
    def test_metrics_snippet_included_in_site_config(self):
        ...
        assert "include /etc/nginx/snippets/metrics.conf" in content, \
...
class TestPrometheusIntegration:
    ...
    def test_nginx_exporter_in_docker_compose(self):
        ...
        assert "nginx/nginx-prometheus-exporter" in content, \
    ...
    def test_nginx_exporter_scrape_uri_configured(self):
        ...
        assert "--nginx.scrape-uri" in content or "nginx.scrape-uri" in content, \
    ...
    def test_prometheus_scrapes_nginx_exporter(self):
        ...
        assert "nginx_exporter:9113" in content, \
...
class TestDockerComposeConfiguration:
    ...
    def test_nginx_logs_volume_mounted(self):
        ...
        assert "nginx_logs:/var/log/nginx" in content, \
    ...
    def test_nginx_snippets_mounted(self):
        ...
        assert "./docker/nginx/snippets:/etc/nginx/snippets:ro" in content, \
...
class TestLoggingDocumentation:
    ...
    def test_logging_snippet_has_comments(self):
        ...
        assert "response time" in content.lower() or "timing" in content.lower(), \
...
class TestRequirementCompliance:
    ...
    def test_requirement_22_criterion_9_compliance(self):
        ...
        assert "$request_time" in content or "request_time" in content, \
    ...
    def test_requirement_24_criterion_3_compliance(self):
        ...
        assert "nginx_exporter:9113" in prom_content, \
